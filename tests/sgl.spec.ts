...
describe('sgl', () => {
  describe('HashIndex', () => {
    ...
    it('should query hashes correctly', () => {
      for (const [i, hash] of hashes.entries()) {
        const mutated = mutate(hash);
        let result = hashIndex.query(mutated, tolerance);
        if (i < exemptedHashes) {
    ...
    it('should reject non-existent hashes', () => {
      for (let i = 0; i < totalHashes; i++) {
        const nonExistentHash = generateRandomHash();
        const result = hashIndex.query(mutate(nonExistentHash), tolerance);
    ...
    it('should insert correctly', () => {
      ...
      let result = hashIndex.query(hash, tolerance);
      expect(result.kind).to.equal('found');
      result = result as QueryResultFound;
